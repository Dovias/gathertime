...
export const getFriendshipRequests = async (userId: number): Promise<FriendshipRequest[]> => {
  const res = await axios.get<FriendshipRequest[]>(`/friendship/user/${userId}/requests`);
  return res.data;
};